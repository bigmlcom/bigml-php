	...
	class BigML {
		...
		*/
		...
		public static function get_source($sourceId, $queryString=null) 
		{
			...
			*/
			$resource = self::_check_resource($sourceId, $queryString);
            if ($resource == null || $resource['type'] != "source") {
               error_log("Wrong source id");
               return null;
		...
		public static function update_source($sourceId, $data) {
			/*
				Updates a source
			*/
			$resource = self::_check_resource($sourceId);
            if ($resource == null || $resource['type'] != "source") {
               error_log("Wrong source id");
               return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
            ...
            $rest = new BigMLRequest('UPDATE', $resource["id"]);
		...
		public static function delete_source($sourceId) {
			...
			*/
            ...
            $rest = new BigMLRequest('DELETE', $resource["id"]);
			return $rest->getResponse();
		}
		...
		public static function create_dataset($origin_resource, $args=array(), $waitTime=3000, $retries=10) {
			...
			if (is_array($origin_resource)) {
				...
				if ($resourse['type'] == "cluster") {
					if (!array_key_exists("centroid",$args)) {
						// TODO coger centroid from cluster
		...
		public static function get_dataset($datasetId, $queryString=null)
        {
			...
			$resource = self::_check_resource($datasetId, $queryString);
            if ($resource == null || $resource['type'] != "dataset") {
		...
		public static function update_dataset($datasetId, $data) {
			/*
               Update a dataset
             */
			$resource = self::_check_resource($datasetId);
            if ($resource == null || $resource['type'] != "dataset") {
               error_log("Wrong dataset id");
               return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
		...
		public static function get_model($modelId, $queryString=null)
        {
			/*
				...
				If this is a shared model, the username and sharing api key must
           		also be provided.
			*/
			$resource = self::_check_resource($modelId, $queryString);
		...
		public static function update_model($modelId, $data) {

			/*
               Updates a model
            */
			$resource = self::_check_resource($modelId);
            if ($resource == null || $resource['type'] != "model") {
                error_log("Wrong model id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
		...
		public static function get_ensemble($ensembleId, $queryString=null)
        {
			/*
				Retrieves an ensemble.
           		...
           		available at the time it is called. 
			*/
			$resource = self::_check_resource($ensembleId, $queryString);
            if ($resource == null || $resource['type'] != "ensemble") {
		...
		public static function update_ensemble($ensembleId, $data, $waitTime=3000, $retries=10) {
			/*
               Updates a ensemble 
            */
			$resource = self::_check_resource($ensembleId, null, $waitTime=3000, $retries=10);
			if ($resource == null || $resource['type'] != "ensemble") {
                error_log("Wrong ensemble id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
        ...
        public static function delete_ensemble($ensembleId) {
			/*
               Deletes a ensemble 
            */
			$resource = self::_check_resource($ensembleId);
            if ($resource == null || $resource['type'] != "ensemble") {
                error_log("Wrong ensemble id");
                return null;
            } 
		...
		public static function get_prediction($predictionId, $queryString=null)
        {
			...
			$resource = self::_check_resource($predictionId, $queryString);
            if ($resource == null || $resource['type'] != "prediction") {
                error_log("Wrong prediction id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
				error_log($resource['message']);
                return null;
		...
		public static function update_prediction($predictionId, $data) 
		{
			/*
               Updates a  prediction 
            */

			$resource = self::_check_resource($predictionId);
            if ($resource == null || $resource['type'] != "prediction") {
                error_log("Wrong prediction id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
		...
		public static function get_batch_prediction($batchPredictionId, $queryString=null)
        {
            ...
            $resource = self::_check_resource($batchPredictionId, $queryString);
            if ($resource == null || $resource['type'] != "batchprediction") {
                error_log("Wrong batch prediction id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
				error_log($resource['message']);
                return null;
		...
		public static function download_batch_prediction($batchPredictionId, $filename=null){ 
			...
			*/
            ...
            $rest = new BigMLRequest('DOWNLOAD', $resource["id"]);
		...
		public static function update_batch_prediction($batchPredictionId, $data) {
			/*
                Updates a batch prediction 
            */

			$resource = self::_check_resource($batchPredictionId);
            if ($resource == null || $resource['type'] != "batchprediction") {
                error_log("Wrong batch prediction id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
		...
		public static function get_evaluation($evaluationId, $queryString=null)
        {
			/*
				...
				The evaluation parameter should be a string containing the
           		...
           		available at the time it is called. 
			*/
			$resource = self::_check_resource($evaluationId, $queryString);
            if ($resource == null || $resource['type'] != "evaluation") {
                error_log("Wrong evaluation id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
				error_log($resource['message']);
                return null;
		...
		public static function update_evaluation($evaluationId, $data) {
			/*
				Updates a evaluation
			*/
			$resource = self::_check_resource($evaluationId);
            if ($resource == null || $resource['type'] != "evaluation") {
                error_log("Wrong evaluation id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
		...
		public static function get_cluster($clusterId, $queryString=null)
        {
			/*
				Retrieves a cluster.
           		...
           		also be provided.
			*/
			$resource = self::_check_resource($clusterId, $queryString);
		...
		public static function update_cluster($clusterId, $data) {
		...
		public static function get_centroid($centroId, $queryString=null)
        {
			...
			*/
			$resource = self::_check_resource($centroId, $queryString);
            if ($resource == null || $resource['type'] != "centroid") {
                error_log("Wrong centroid id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
  				error_log($resource['message']);
                return null;
		...
		public static function update_centroid($centroId, $data) {

			/*
               Updates a centroid.
            */
			$resource = self::_check_resource($centroId);
            if ($resource == null || $resource['type'] != "centroid") {
                error_log("Wrong centroid id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
		...
		public static function get_batchcentroid($batchcentroId, $queryString=null)
        {
			/*
				Retrieves a batch centroid.
           		...
           		info available at the time it is called.
			*/
			$resource = self::_check_resource($batchcentroId, $queryString);
            if ($resource == null || $resource['type'] != "batchcentroid") {
                error_log("Wrong batch centroid id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
				error_log($resource['message']);
                return null;
		...
		public static function update_batchcentroid($batchcentroId, $data) {
			/*
				Updates a batch centroid.
			*/
			$resource = self::_check_resource($batchcentroId);
            if ($resource == null || $resource['type'] != "batchcentroid") {
                error_log("Wrong batch centroid id");
                return null;
            } elseif ($resource["status"] != BigMLRequest::FINISHED) {
