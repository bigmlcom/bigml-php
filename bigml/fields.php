      ...
      if (is_array($input_data)) {
         foreach($input_data as $name => $value) {
           if (array_key_exists($name, $this->fields_by_name)) {
           	   $a = "[" . $name  . str_repeat(utf8_encode(' '), 32) . ":" ;
           	   $a = $a . gettype($input_data[$name]) ;
           	   $a = $a . str_repeat(utf8_encode(' '), 16) . ":";
           	   $a = $a . $this->fields->{$this->fields_by_name[$name]}->optype;
           	   $a = $a . str_repeat(utf8_encode(' '), 16) . ":";

              if (in_array(gettype($input_data[$name]),
              	  php_map_type($this->fields->{$this->fields_by_name[$name]}->optype) )) {
                $a = $a . "OK\n"; 
              } else {
                $a = $a . "WRONG\n";
              }
